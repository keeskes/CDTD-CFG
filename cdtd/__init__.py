...
class MixedTypeDiffusion(nn.Module):
    ...
    def loss_fn(self, x_cat, x_cont, u=None):
        ...
        cat_logits, cont_preds = self.precondition(x_cat_emb_t, x_cont_t, u, sigma) # Model outputs based on noised data, u and sigma
    ...
    def precondition(self, x_cat_emb_t, x_cont_t, u, sigma):
        ...
        # Inputs are passed through the model, where the cat and cont embeddings are rescaled, and the noise level is added
    ...
    def fit(
        ...
        with tqdm(
            ...
            while self.current_step < num_steps_train:
                ...
                losses, _ = self.diff_model.loss_fn(x_cat, x_cont, None) # Compute the losses (CE, MSE, timewarp, weightnet)
